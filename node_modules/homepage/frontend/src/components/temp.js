...
function App() {
    ...
    // This function sends the time data to the server
    const submitTime = async (userSessionId, questionNumber, timeSpent) => {
        try {
            await fetch('http://localhost:3000/submit-time', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({ userSessionId, questionNumber, timeSpent }),
            });
        } catch (error) {
